...
class Crawler(Spider):
    ...
    def parse_job_microdata(self, response):
        ...
        for job in jobs:
            job = self.normalize(job, job_url)
            for field, selector in job_selectors.items():
                job[field] = ','.join(text.strip() for text in response.xpath(selector + '/text()').getall() if text is not None)
            yield job
    ...
    def get_data_reduction(uri, database, collection):
        ...
        jobs = list(collection.find({},
                                    {'title': 1, 'hiringOrganization.name': 1, 'jobLocation.address.addressRegion': 1,
                                     '_id': 0}))
        data = [
            [job['title'], job['hiringOrganization']['name'], ','.join(job['jobLocation']['address']['addressRegion'])]
            for job in jobs]
